...
#Description: Classes for the different CNN-based DNN architectures with 7x7 kernels

import torch
from torch import nn
import torch.nn.functional as F

#single convolutional layer 7x7 kernel LeNet CNN with CIFAR-10 dataset
...
#5-convolutional-layer 7x7 kernel CNN with CIFAR-10 dataset
class CNN_Conv_5_CIFAR10(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(32, 64, 7, padding=2)
        #convolutional layer 5 with 64 input channels and 128 output feature maps
        self.conv5 = nn.Conv2d(64, 128, 7, padding=2)
        #2x2 max pooling kernel
        self.pool = nn.MaxPool2d(2, 2)
        #fully connected layer 1 with 120 outputs
    ...
    def forward(self, x):
        #convolutional layer2 with ReLU and max pooling
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        x = self.pool(F.relu(self.conv3(x)))
...
#7-convolutional-layer 7x7 kernel CNN with CIFAR-10 dataset
class CNN_Conv_7_CIFAR10(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 6, 7, padding=3)
        #convolutional layer 2 with 6 input channels and 16 output feature maps
        self.conv2 = nn.Conv2d(6, 16, 7, padding=3)
        #convolutional layer 3 with 16 input channels and 32 output feature maps
        ...
        self.conv6 = nn.Conv2d(128, 256, 7, padding=3)
...
#VGG 7x7 kernel CNN with CIFAR-10 dataset
